...
namespace Xamarin.Android.Tools.BootstrapTasks
{
	public class RunAndroidEmulatorCheckBootTimes : Task
	{
		...
		static void CloseProcesses ()
		{
			...
			for (int i = 0; i < 10; i++) {
				try {
					var proc = System.Diagnostics.Process.GetProcesses ().FirstOrDefault (p => p.ProcessName.IndexOf ("qemu") != -1 || p.ProcessName.IndexOf ("emulator") != -1);
			...
			for (int i = 0; i < 10; i++) {
				try {
					var proc = System.Diagnostics.Process.GetProcesses ().FirstOrDefault (p => p.ProcessName.IndexOf ("check-boot-times") != -1);
