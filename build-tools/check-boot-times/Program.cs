	...
	class Program
	{
		...
		static int Main (string [] args)
		{
			var app = new CommandLineApplication {
				Name = "CheckBootTimes",
				Description = "Collect Android Emulator boot times."
			};

			app.HelpOption ("-? | -h | --help");

			var argExecutionTimes = app.Option ("-ExecutionTimes", "Number of times emulator should run.", CommandOptionType.SingleValue);
			var argSdkPath = app.Option ("-SdkPath", "Android sdk location.", CommandOptionType.SingleValue);
			var argAdbPath = app.Option ("-AdbPath", "Adb path location including Adb file.", CommandOptionType.SingleValue);
			var argAvdManagerPath = app.Option ("-AvdManagerPath", "AvdManager path location including AvdManager file.", CommandOptionType.SingleValue);
			var argEmulatorPath = app.Option ("-EmulatorPath", "Emulator path location including Emulator file.", CommandOptionType.SingleValue);
			var argSdkManagerPath = app.Option ("-SdkManagerPath", "SdkManager path location including SdkManager file.", CommandOptionType.SingleValue);
			var argDeviceName = app.Option ("-DeviceName", "Emulator Virtual Device Name, If not provided a new one will be created.", CommandOptionType.SingleValue);
			var argVerbose = app.Option ("-v|--verbose", "Show verbose output", CommandOptionType.NoValue);

			app.OnExecute (async () => {
				if (argExecutionTimes.HasValue () && !int.TryParse (argExecutionTimes.Values [0], out executionTimes)) {
					Console.WriteLine ($"ExecutionTimes argument should be integer. It represents number of times we want to run emulator. If not provided default value equals 1");
					return 1;
				}

				if (argAdbPath.HasValue () && !File.Exists (argAdbPath.Values [0])) {
					Console.WriteLine ($"Unable to find adb. File does not exist '{argAdbPath.Values [0]}'");
					return 1;
				}

				if (argAvdManagerPath.HasValue () && !File.Exists (argAvdManagerPath.Values [0])) {
					Console.WriteLine ($"Unable to find AvdManager. File does not exist '{argAvdManagerPath.Value ()}'");
					return 1;
				}

				if (argEmulatorPath.HasValue () && !File.Exists (argEmulatorPath.Value ())) {
					Console.WriteLine ($"Unable to find Emulator. File does not exist '{argEmulatorPath.Value ()}'");
					return 1;
				}

				if (argSdkManagerPath.HasValue () && !File.Exists (argSdkManagerPath.Value ())) {
					Console.WriteLine ($"Unable to find SdkManager. File does not exist '{argSdkManagerPath.Value ()}'");
					return 1;
				}

				sdkPath = argSdkPath.Value ();
				adbPath = argAdbPath.Value ();
				avdManagerPath = argAvdManagerPath.Value ();
				emulatorPath = argEmulatorPath.Value ();
				sdkManagerPath = argSdkManagerPath.Value ();
				deviceName = argDeviceName.Value ();
				verbose = argVerbose.HasValue ();
		...
		static async Task<bool> CloseEmulator ()
		{
			...
			for (int i = 0; i < 10; i++) {
				...
				try {
					var proc = System.Diagnostics.Process.GetProcesses ().FirstOrDefault (p => p.ProcessName.IndexOf ("emulator") != -1 || p.ProcessName.IndexOf ("qemu") != -1);
